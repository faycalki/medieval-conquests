...
#Added V1.001C11: mission_tpl_are_all_agents_spawned in can_fail_operations, and as an operation move, as well as uncommented it in the mission templates.
...
# DUPLICATE server_set_max_num_players   = 491 #( , <max_players>, [<max_private_players>]), #Sets maximum players to <max_players> and maximum private players to [<max_private_players>] (default = same as <max_players>). Both values must be in the range 2-250, [<max_private_players>] can't be lower than <max_players>
...
#ENVFIX BUGS OUT NOT NEEDED str_store_face_keys                       = 2626 #(str_store_face_keys, <string_register>, <face_key_register>), #Stores the string representation of <face_key_register> into <string_register>
...
str_store_server_password_rcon            = 2638 #(str_store_server_password_rcon, <string_register>), #Stores the server RCON password into <string_register>
str_store_item_mesh_name                  = 2639 #(str_store_item_mesh_name, <string_register>, <item_no>), #Stores the mesh name of <item_no> into <string_register>
...
player_set_username           = 2905 #(player_set_username, <player_no>, <string_no>), #Sets <player_no>'s username to <string_no>
...
agent_body_meta_mesh_set_vertex_keys_time_point = 3319 #(agent_body_meta_mesh_set_vertex_keys_time_point, <agent_no>, <body_meta_mesh>, <time_point>), #Sets <agent_no>'s <body_meta_mesh> vertex keys time point to <time_point>
agent_body_meta_mesh_set_visibility             = 3320 #(agent_body_meta_mesh_set_visibility, <agent_no>, <body_meta_mesh>, <value>), #Shows (<value> = 1) or hides (<value> = 0) <agent_no>'s <body_meta_mesh>
...
missile_is_valid                 = 3619 #(missile_is_valid, <missile_no>), #Fails if <missile_no> is not valid
missile_get_cur_position         = 3620 #(missile_get_cur_position, <position_register>, <missile_no>), #Stores <missile_no>'s current position into <position_register>
...
#ENVFIX BUGS OUT NOT NEEDED troop_get_face_keys          = 3710 #(troop_get_face_keys, <face_key_register>, <troop_no>, [<alt>]), #Stores <troop_no>'s face keys into <face_key_register>. if [<alt>] is non-zero the second pair of face keys is stored
#ENVFIX BUGS OUT NOT NEEDED troop_set_face_keys          = 3711 #(troop_set_face_keys, <troop_no>, <face_key_register>, [<alt>]), #Sets <troop_no>'s face keys to <face_key_register>. if [<alt>] is non-zero the second pair of face keys is set
...
#Envfix bugfix coop tree_box errors (((item kind))) item_set_swing_damage        = 3830 #(item_set_swing_damage, <item_kind_no>, <value>), #Sets <item_kind_no>'s thrust damage to <value>
#Possible ENFIX item_set_swing_damage_type   = 3831 #(item_set_swing_damage_type, <item_kind_no>, <value>), #Sets <item_kind_no>'s thrust damage type to <value>
...
set_shader_param_int    = 4705 #(set_shader_param_int, <parameter_name>, <value>), #Sets the int shader parameter <parameter_name> to <value>
#Envfix, this shader set param bugs the game. do not re-enable. set_shader_param_float  = 4701 #(set_shader_param_float, <parameter_name>, <value_fixed_point>), #Sets the float shader parameter <parameter_name> to <value_fixed_point>
#Functions below lost 1 (-1) number due to disabled shader.
set_shader_param_vector = 4701 #(set_shader_param_vector, <parameter_name>, <position_register>), #Sets the float4 shader parameter <parameter_name> to <position_register>'s origin
set_shader_param_matrix = 4702 #(set_shader_param_matrix, <parameter_name>, <position_register>), #Sets the float4x4 shader parameter <parameter_name> to <position_register>
...
#Patched to an extent Maybe remove edit_mode above if abuseable in co-op/MP.
